 ...
 * whichever bucket sits at its current dock (see FleetOwner.crewFleet/
...
import type { FleetOwner } from "./faction";
 ...
 * whose `piracy` is at most `maxPiracy` (see FleetOwner.hirePiracyThreshold) --
 ...
 * FleetOwner.fillExtraSeats/Captain.hireCrewIfPossible). Defaults to 1.0 (hire
...
/** Final per-Location pool size is scaled up by this factor -- a buffer against FleetOwner.hirePiracyThreshold shrinking the effective merchant/police-hireable pool over time as piracy-tainted Sailors accumulate (pirates themselves already drew from this same pool before that filter existed; this isn't about new pirate demand). */
 ...
 * summed across every already-registered FleetOwner's Ships (Company/SoloTrader/
 * PirateBrigade/PoliceFleet), counting only seats beyond the Captain (a
 * fresh FleetOwner's transport.crew is always just [captain] at this point --
 * see FleetOwner's constructor/crewFleet). Must run after every initial
 * FleetOwner/Ship has been constructed (so `transport.location`/
 * `crewRequirement` are set) but BEFORE any FleetOwner.crewFleet() call, so the
 * pool exists by the time crewing actually draws from it. Called once by
 * World's constructor.
 */
export function generateSailorPool(factions: readonly FleetOwner[]): void {
