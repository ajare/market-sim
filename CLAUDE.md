...
- **Weather and storms** (`weather.ts`/`storms.ts`, Architecture.md §18): opt-in — only a `World` built from an editor-authored JSON with a `weatherProfile` has any of this; `buildWorld()`'s procedural default never constructs a `WeatherSystem`. `WeatherSystem` is a pure, stateless function of `(timeOfYear, position)` (temperature/wind speed+direction/rainfall), shaped by a `WeatherProfile` (`WEATHER_PROFILES.default`/`.caribbean`). `StormSystem` layers stateful `Storm` entities on top — spawn odds driven by rainfall², movement follows local wind + wobble, intensity ramps/plateaus/decays, escalating to a cyclone under sustained warm/tropical/high-intensity conditions (`setStormFrequency`/`getStormFrequency` tune the global spawn rate). Each `Captain.act()` samples wind and any storm at its **current port, once, for that day only** (`currentDay`/`currentWeather`/`currentStorms`, never resampled mid-voyage): wind speeds up or slows down travel time (`windSpeedMultiplier`), and departing into a storm applies both a speed penalty and a one-time `Transport.condition` hit (`applyStormDamageOnDeparture`) tagged into `Transport.conditionHistory` (cause `"transit"`/`"pirate"`/`"storm"`/`"repair"`) for the UI's condition chart.
